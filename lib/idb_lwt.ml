...
type store = {
  ...
  (* We reuse transactions where possible for performance.
     ...
     This does mean that if any read fails then the others will hang,
     but we treat any read failing as a fatal error anyway. *)
  mutable ro_trans :
    ...
    (fun target ->
       Js.Opt.case (target##error)
         ...
         (fun error ->
            ...
            let message =
              opt_string (error##message) ~if_missing:"(no message)" in
...
let rec trans_ro (t:store) setup =
  ...
  | None ->
    ...
    r
  | Some (trans, breakers) ->
    (* Seems we can get here when a transaction is done but oncomplete
       hasn't been called, so retry if we get an error. *)
    try
      setup (trans##objectStore (t.store_name)) set_r;
...
let set t key value =
  trans_rw t (fun store ->
      store##put (Js.string (UTF8_codec.encode value), Js.string key)
      |> ignore
    )

let remove t key =
  trans_rw t
    (fun store -> store##delete (Js.string key) |> ignore)
       ...
       request##onsuccess <- Dom.handler (fun _event ->
           ...
           if test actual then (
             begin
               ...
               | None ->
                 store##delete (key) |> ignore
               | Some new_value ->
                 store##put (Js.string (UTF8_codec.encode new_value), key)
                 |> ignore
             end;
             result := Some true
           ) else (
